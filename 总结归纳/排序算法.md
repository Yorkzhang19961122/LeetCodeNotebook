...
public:
    ...
    void adjustHeap(vector<int>& arr, int k, int end) {
        ...
        while(2 * k + 1 <= end) {  //2k+1 <= len意味着左子节点存在
            ...
            else {  //否则说明此小分支满足大顶堆的要求，退出循环
                break;
            }
            k = j;  //交换完成后将k更新为j，因为不能保证当前的子节点是它子树的最大值，所以需要继续向下比较
        }
    }
};
```
...
* 记录原数组中的`max`和`min`
* 创建`max-len + 1`这么大跨度的数组`count`用于统计原数组中各数字出现的次数（下标为数字，值为出现的次数）
...
public:
    ...
    void countingSort(vector<int>& arr) {
        ...
        //创建max-len这么大跨度的数组count用于统计原数组中各数字出现的次数（下标为数字，值为出现的次数）
        int len = max - min + 1;
